...
import "../assets/styles/base/tilt.css";
import "../assets/styles/base/tilt-intro.css";
import "../assets/styles/browser/tabs.css";
import "../assets/styles/browser/rivet.css";

const recoveryUrl = new URL(window.location.href);
if (recoveryUrl.searchParams.has("tilt-recovery")) {
  recoveryUrl.searchParams.delete("tilt-recovery");
...
window.__tiltStuffData = fetch("/api/stuff", { cache: "no-store" })
  .then((response) => (response.ok ? response.json() : null))
  .then((serviceMetadata) => {
    if (serviceMetadata?.turn) {
      (
        window as typeof window & { __TILT_WEBRTC_TURN__?: unknown }
      ).__TILT_WEBRTC_TURN__ = serviceMetadata.turn;
    }
...
function initializeTiltApp() {
  const runtimeWindow = window as typeof window & {
    __tiltAppInitialized?: boolean;
  };
  const appRoot = document.querySelector<HTMLElement>(".meow");
  if (
    !appRoot ||
    runtimeWindow.__tiltAppInitialized ||
    appRoot.dataset.tiltAppInitialized === "true"
  ) {
    return;
  }
  runtimeWindow.__tiltAppInitialized = true;
  appRoot.dataset.tiltAppInitialized = "true";

  initUiSignals();
  initCloaking();
  store.setupWindowTilt();
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeTiltApp, {
    once: true,
  });
} else {
  initializeTiltApp();
