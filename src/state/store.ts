...
export const store = {
  ...
  setupWindowTilt() {
    ...
    window.Tilt.tabs = this.tabs as any;
    window.Tilt.splitPair = this.splitPair;
    window.Tilt.isLoading = false;
    window.Tilt.allGames = this.allGames;
    window.Tilt.getActiveTab = () => this.getActiveTab();
    window.Tilt.getActivePageState = () => this.getActivePageState();
    window.Tilt.getTabPageState = (tabId: number) =>
      this.getTabPageState(tabId);
    window.Tilt.setPlayerStatus = (tabId: number, status: string) => {
      if (isPlayerStatus(status)) this.setPlayerStatus(tabId, status);
    };
    window.Tilt.renderTabs = () => this.notify();
    window.Tilt.resetSession = () => this.resetSession();
    window.Tilt.openNewTabFromServiceWorker = (url: string | null, options: { title?: string; openerTabId?: number } = {}) => {
      if (!url) return null;
      const tab = this.addTab(url, options.title || "fetching data...");
      if (tab && options.openerTabId) tab.openerTabId = options.openerTabId;
      return tab;
    };
    window.Tilt.handleSearch = async (query: string, gameName?: string, gameIcon?: string | null) => {
      const activeTab = this.getActiveTab();
      if (activeTab) {
