type TiltBuildPayload = {
  build?: unknown;
};

type TiltRuntimeWindow = Window & {
  __tiltStuffData?: Promise<TiltBuildPayload | null>;
};
...
function fetchBuildPayload(): Promise<TiltBuildPayload | null> {
  if (typeof fetch !== "function" || typeof location === "undefined") {
    return Promise.resolve(null);
  }
  return fetch("/api/stuff", { cache: "no-store" })
    .then((response) =>
      response.ok ? (response.json() as Promise<TiltBuildPayload>) : null,
    )
    .catch(() => null);
}
...
function getServerNamespace(): Promise<string | null> {
  ...
  const runtimeWindow =
    typeof window === "undefined"
      ? null
      : (window as TiltRuntimeWindow);
  const buildPromise =
    runtimeWindow?.__tiltStuffData ??
    (runtimeWindow ? (runtimeWindow.__tiltStuffData = fetchBuildPayload()) : null);
