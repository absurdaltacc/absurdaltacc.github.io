...
type IframeHistoryManager = NonNullable<TiltTab["historyManager"]>;
...
function syncGlobalLoadingState(): void {
  window.Tilt.isLoading = Boolean(
    window.Tilt.tabs?.some((tab: TiltTab) => tab.isLoading),
  );
}
...
function finishGameLoadMetric(
  ...
  const tab = window.Tilt?.tabs?.find((candidate) => candidate.iframe === iframe);
...
function failGameNavigation(
  ...
  const tab = window.Tilt?.tabs?.find((candidate) => candidate.iframe === iframe);
  if (tab?.isGame) window.Tilt.setPlayerStatus?.(tab.id, "error");
...
function applyPageStateToTab(tab: TiltTab): boolean {
...
async function waitForFolioNavigationTransport(): Promise<void> {
  if (!window.Tilt?.waitForTransport) return;
  try {
    await window.Tilt.waitForTransport(8000);
  } catch (e: unknown) {
    console.warn(
    ...
    try {
      ...
      await window.Tilt.waitForTransport?.(6000);
    } catch (recoveryError) {
      console.warn(
...
function detachContentWindowListeners(iframe: HTMLIFrameElement): void {
  try {
    ...
    if (typeof iframeWindow.__tiltHistoryCleanup === "function") {
      iframeWindow.__tiltHistoryCleanup();
      delete iframeWindow.__tiltHistoryCleanup;
    }
    ...
    if (iframeWindow.__tiltFocusHandler) {
      iframeWindow.removeEventListener(
        "mousedown",
        iframeWindow.__tiltFocusHandler as EventListener,
        true,
      );
      iframeWindow.__tiltFocusHandler = null;
    }
  } catch (e) {
...
export function stopIframeLoading(iframe: HTMLIFrameElement): void {
  ...
  const currentTab = window.Tilt.tabs?.find((tab) => tab.id === tabId);
  ...
  const tab = window.Tilt?.tabs?.find((tab) => tab.iframe === iframe);
...
export function navigateIframeTo(iframe: HTMLIFrameElement, url: string, mode: "push" | "replace" = "push"): void {
  ...
  const iframeExt = iframe as HTMLIFrameElement & Record<string, unknown>;
  const tab = window.Tilt.tabs!.find((t) => t.iframe === iframe);
  tab?.historyManager?.begin(mode);
  iframe.dataset.navigationVersion = String(navigationVersion);
  const historyManager = iframeExt.__tiltHistoryManager as
  ...
  if (historyManager) {
    installIframeLoadHandlers(
      ...
      historyManager,
      tab?.id ??
        (iframeExt.__tiltTabId as number | null | undefined) ??
  ...
  if (tab) {
    ...
    if (window.Tilt.renderTabs)
      (window.Tilt.renderTabs as () => void)();
  }
  ...
  } else if (isProxyUrl && window.Tilt?.waitForTransport) {
    (async () => {
      try {
        await window.Tilt.waitForTransport?.(8000);
      } catch (e: unknown) {
...
export function clearExtensionPageForNavigation(
  ...
  const tab =
    tabOverride ??
    window.Tilt.tabs?.find((candidate) => candidate.iframe === iframe);
...
export function cleanupIframe(iframe: HTMLIFrameElement): void {
  ...
  window.Tilt?.releaseFolioFrame?.(iframe);
  clearExtensionPageForNavigation(iframe);
  const iframeExt = iframe as HTMLIFrameElement & Record<string, unknown>;
  clearGameLoadState(iframe, true);
  const handlers = iframeExt.__tiltInternalHandlers as
  ...
  if (handlers) {
    ...
    iframeExt.__tiltInternalHandlers = null;
  }
...
function updateTabDetails(iframe: HTMLIFrameElement): void {
  const tabToUpdate = window.Tilt.tabs!.find(
    (tab) => tab.iframe === iframe,
  );
  if (!tabToUpdate) return;
  const usedPageState = applyPageStateToTab(tabToUpdate);
  if (usedPageState) {
    if (window.Tilt.renderTabs) (window.Tilt.renderTabs as () => void)();
    return;
  }
  ...
  try {
    ...
    if (
      ...
      const isActiveTab = document.body.classList.contains("split-view")
        ? tabToUpdate.id === (window.Tilt as any)?.splitPair?.left ||
          tabToUpdate.id === (window.Tilt as any)?.splitPair?.right ||
          tabToUpdate.id === (window.Tilt as any)?.getActiveTab?.()?.id
        : tabToUpdate.id === (window.Tilt as any)?.getActiveTab?.()?.id;
    ...
    if (!isReloading && window.Tilt.renderTabs)
      (window.Tilt.renderTabs as () => void)();
  }
}
...
function setupIframeContentListeners(
  iframe: HTMLIFrameElement,
  historyManager: NonNullable<TiltTab["historyManager"]>,
  tabId: number | null,
): void {
  try {
    const iframeWindow = iframe.contentWindow;
    ...
    const hasPageStatePlugin = Boolean((iframeWindow as Window & { __tiltFolioPageStateInstalled?: boolean }).__tiltFolioPageStateInstalled);
    if (navigation && !hasPageStatePlugin) {
      ...
      (iframeWindow as Window & Record<string, unknown>).__tiltHistoryCleanup = () =>
    ...
    const beforeUnloadHandler = () => {
      ...
      window.Tilt.isLoading = true;
      iframe.parentElement?.classList.remove("loaded");
      const tab = window.Tilt.tabs!.find((t) => t.id === tabId);
      if (tab) {
        if (!tab.fixedTitle) {
          tab.title = "fetching data...";
        }
    ...
    (iframeWindow as Window & Record<string, unknown>).__tiltFocusHandler =
      mouseDownHandler;
...
export function updateHistoryUI(
  ...
  getNavEls();
  const stillExists =
    activeTab && window.Tilt?.tabs?.some((tab) => tab.id === activeTab.id);
...
function installIframeLoadHandlers(
  ...
  const iframeExt = iframe as HTMLIFrameElement & Record<string, unknown>;
  const previousHandlers = iframeExt.__tiltInternalHandlers as
    | IframeInternalHandlers
    | null
    | undefined;
  if (previousHandlers) {
  ...
  const onLoad = (): void => {
    ...
    if (tab?.isGame && tab.playerStatus === "error") {
      window.Tilt.setPlayerStatus?.(tab.id, "idle");
    }
    ...
    try {
      ...
      if (nativeUrl && nativeUrl !== "about:blank") {
        ...
        if (!(win as Window & { __tiltFolioPageStateInstalled?: boolean }).__tiltFolioPageStateInstalled) {
  ...
  iframeExt.__tiltInternalHandlers = {
    onError,
    onLoad,
...
export function initializeIframe(
  ...
  const iframeExt = iframe as HTMLIFrameElement & Record<string, unknown>;
  iframeExt.__tiltHistoryManager = historyManager;
  iframeExt.__tiltTabId = tabId;
  installIframeLoadHandlers(
    iframe,
    historyManager,
    tabId,
