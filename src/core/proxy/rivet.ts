...
type TiltTab = (typeof store.tabs)[number];
...
function tabUrl(tab: TiltTab): string {
  return (
    tab.pageState?.url ||
...
function toTabInfo(tab: TiltTab): TabInfo {
  return {
    id: tab.id,
    windowId: 1,
    url: tabUrl(tab),
...
function tabForWindow(win: Window): TiltTab | null {
...
export async function initializeRivet(): Promise<RivetFacade> {
  ...
  const pending = (async () => {
    ...
    registerRivetBridge(rivet, (tab) =>
      mountRivetNewTabOverride(tab as TiltTab),
    );

    const tilt = (window.Tilt ??= {} as typeof window.Tilt);
    (tilt as typeof tilt & { rivet: RivetFacade }).rivet = rivet;
...
function mountRivetNewTabOverride(tab: TiltTab): boolean {
  const rivet = instance;
