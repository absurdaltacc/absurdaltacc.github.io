...
declare global {
  interface Window {
    tiltConnection: TiltConnectionManager;
  }
}
...
class TiltConnectionManager {
  ...
  constructor() {
    ...
    const app = ((window as unknown as Record<string, unknown>)["Tilt"] ??= {}) as typeof window.Tilt;
  ...
  _resetTransportReady(): void {
    if (this._transportReadyPromise._settled) {
      this._transportReadyPromise = createResolvablePromise();
      window.Tilt.transportReady = this._transportReadyPromise;
    }
  }
  ...
  async initializeApp(isRetry: boolean = false): Promise<boolean | undefined> {
    ...
    try {
      if (!this.bareMuxConnection) {
        ...
        window.Tilt.bareMuxConnection = this.bareMuxConnection;
      }
  ...
  startHealthCheck(): void {
    ...
    this.healthCheckInterval = setInterval(async () => {
      ...
      const hasLoadingTab = window.Tilt?.tabs?.some((tab) => tab.isLoading);
      if (window.Tilt?.isLoading || hasLoadingTab) return;
  ...
  setupEventListeners(): void {
    window.addEventListener("message", (ev: MessageEvent) => {
      ...
      if (!message || message.type !== "tilt-prefetch-bridge") return;
      ...
      if (!ctrl) return;
      try {
        ctrl.postMessage({ type: "tilt-prefetch", url: message.url });
      } catch (e) {
...
window.tiltConnection = new TiltConnectionManager();
export {};
