...
const SUBTITLE_PREFERENCE_KEY = "tilt-anime-subtitle";
...
export default function Player() {
  ...
  const baseVideoSrc =
    ...
    hasMegaPlayIdentifier(playbackIds)
      ...
      : "";
  const resumeKey = `tilt-resume-anikoto-${animeIdentityCacheKey({ ids: playbackIds })}-${episodeNumber}-${language}`;
  ...
  const [mediaState, setMediaState] = useState<MediaState>(() => {
    const storedVolumeValue = localStorage.getItem("tilt-anime-volume");
    const storedVolume =
  ...
  const reportPlayerStatus = useCallback((status: PlayerStatus) => {
    try {
      const parentTilt = window.parent?.Tilt;
      const tabId = parentTilt?.tabs?.find(
        (tab) => tab.iframe?.contentWindow === window,
      )?.id;
      if (typeof tabId === "number") {
        parentTilt?.setPlayerStatus?.(tabId, status);
      }
    } catch {
  ...
  const handleVolume = useCallback((event: Event) => {
    ...
    video.muted = false;
    localStorage.setItem("tilt-anime-volume", String(volume));
    syncMediaState();
  }, [syncMediaState]);
    ...
    const handleKeydown = (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "ArrowUp":
          ...
          video.muted = false;
          localStorage.setItem("tilt-anime-volume", String(Math.min(100, volume + 5)));
          syncMediaState();
          break;
        case "ArrowDown":
          event.preventDefault();
          ...
          video.muted = false;
          localStorage.setItem("tilt-anime-volume", String(Math.max(0, volume - 5)));
          syncMediaState();
          break;
        case "m":
          toggleMute();
  ...
  useEffect(() => {
    ...
    const parentTilt = (window.parent as any)?.Tilt;
    if (parentTilt?.tabs) {
      const myTab = parentTilt.tabs.find(
        (t: any) => t.iframe?.contentWindow === window,
      );
      if (myTab) {
        if (title) myTab.title = title;
        if (poster) myTab.favicon = poster;
        parentTilt.renderTabs?.();
      }
    }
  }, [title, poster]);
