...
export default function GamesCatalog({
  ...
  const { visible, active, searchBarRef, show, hide, toggle } = useMenuView({
    ...
    openedStorageKey: "tiltUserOpenedGameMenu",
  ...
  useEffect(() => {
    ...
    window.addEventListener("tilt:game-catalog-updated", handler);
    return () => {
      window.removeEventListener("tilt:game-catalog-updated", handler);
  ...
  const placeholder = loaded
    ? `search through ${allGames.length} games...`
    : "fetching games...";
