...
function getBookmarks(): Bookmark[] {
  try {
    const raw = localStorage.getItem("tilt-bookmarks");
    if (!raw) {
      const defaults = DEFAULT_BOOKMARKS.slice();
      localStorage.setItem("tilt-bookmarks", JSON.stringify(defaults));
      return defaults;
    }
...
function saveBookmarks(bookmarks: Bookmark[]) {
  localStorage.setItem("tilt-bookmarks", JSON.stringify(bookmarks));
}
...
export default function Bookmarks() {
  ...
  useEffect(() => {
    ...
    const storageHandler = (e: StorageEvent) => {
      if (e.key === "tilt-bookmarks") reload();
    };
          ...
          {bookmarks.map((bookmark, index) => (
            <li key={bookmark.url + index} class="bookmark-item">
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  (window.Tilt as any)?.handleSearch(bookmark.url);
                }}
              >
