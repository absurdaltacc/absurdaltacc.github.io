...
function replaceGames(games: readonly GameEntry[]): boolean {
  ...
  window.Tilt.allGames = store.allGames;
  store.notify();
  return true;
}
...
export default function NewTabModal({
  ...
  useEffect(() => {
    ...
    window.addEventListener("tilt:game-catalog-updated", handleCatalogUpdated);
    document.addEventListener("gameSourceUpdated", handleSourceUpdated);
    return () => {
      window.removeEventListener("tilt:game-catalog-updated", handleCatalogUpdated);
