...
const GAME_CACHE_KEY_PREFIX = "tilt-game-cache";
...
function setAllGames(source: GameSourceKey, games: GameEntry[]): GameEntry[] {
  ...
  try {
    const tilt = (window as unknown as {
      Tilt?: { allGames?: GameEntry[] };
    }).Tilt;
    if (tilt) {
      if (Array.isArray(tilt.allGames)) {
        tilt.allGames.splice(0, tilt.allGames.length, ...games);
      } else {
        tilt.allGames = games;
      }
    }
...
function notifyCatalogUpdated(source: GameSourceKey, games: GameEntry[]): void {
  try {
    window.dispatchEvent(
      new CustomEvent("tilt:game-catalog-updated", {
        detail: { source, games },
      }),
    );
  } catch {}
