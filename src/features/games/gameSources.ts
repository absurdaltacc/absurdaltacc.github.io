...
const AETHERIS_CATALOG_URL =
  "https://raw.githubusercontent.com/mynamescrax/aetheris/main/public/assets/data/aetheris.json";
...
function parseAetheris(payload: unknown): GameEntry[] {
  const rows = arrayPayload(payload, "aetheris");
  return rows.flatMap((game) => {
    const name = text(game.title) || text(game.name);
    const gameUrl = absoluteHttpUrl(
      game.url ?? game.html,
      SOURCE_ORIGINS.aetheris,
    );
    if (!name || !gameUrl) return [];
    const image = absoluteHttpUrl(
      game.image ?? game.img ?? game.icon,
      SOURCE_ORIGINS.aetheris,
    );
    const mapped = mappedGame("aetheris", {
      id: identifier(game.id, gameUrl),
      name,
      gameUrl,
      coverUrl: image ? getProxyUrl(image) : "",
      author: text(game.author),
      featured: booleanValue(game.featured) || booleanValue(game.popular),
    });
    return mapped ? [mapped] : [];
  });
}
...
export const GAME_SOURCE_ADAPTERS: Record<GameSourceKey, GameSourceAdapter> = {
  aetheris: {
    key: "aetheris",
    catalogUrl: AETHERIS_CATALOG_URL,
    parse: parseAetheris,
  },
  edurocks: {
    key: "edurocks",
