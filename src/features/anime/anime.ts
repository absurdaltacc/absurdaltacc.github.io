...
>();
const ANIME_FEED_CACHE_KEY_PREFIX = "tilt-anime-feed-entries";
const ANIME_SEARCH_CACHE_KEY_PREFIX = "tilt-anime-search-entries";
const ANIME_EPISODE_CACHE_KEY_PREFIX = "tilt-anime-episode-count";
...
export function resetAnimeCache(): void {
  ...
  try {
    for (const key of Object.keys(sessionStorage)) {
      if (key.startsWith("tilt-anime-")) sessionStorage.removeItem(key);
    }
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith("tilt-anime-feed-")) localStorage.removeItem(key);
    }
  } catch {
