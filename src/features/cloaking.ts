...
const TILT_APPEARANCE: CloakingAppearance = {
  title: "Absent",
  icon: "/absent-favicon.svg",
};

const SITE_APPEARANCES: Record<string, CloakingAppearance> = {
  none: TILT_APPEARANCE,
  google: {
    title: "Google",
...
function resolveCloakingAppearance(
  ...
  if (focusCloaking && tabActive) return TILT_APPEARANCE;
...
function buildCloakedPage(
  ...
  return `<!doctype html><html><head><title>${title}</title><link rel="icon" href="${icon}"></head><body style="margin:0"><iframe title="tilt" style="height:100%;width:100%;border:0;position:fixed;inset:0" src="${iframeUrl}"></iframe></body></html>`;
}
