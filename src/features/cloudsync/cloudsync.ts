...
export class CloudSync {
  ...
  constructor() {
    ...
    try {
      this.syncMeta = JSON.parse(
        localStorage.getItem("tilt-sync-meta") ||
  ...
  saveMeta(): void {
    localStorage.setItem("tilt-sync-meta", JSON.stringify(this.syncMeta));
  }
  ...
  async checkForChanges(): Promise<void> {
    ...
    if (typeof window.tiltExportAllData !== "function") return;
    this._isScanning = true;
    try {
      const fingerprint = await snapshotFingerprint(
        await window.tiltExportAllData(),
      );
  ...
  async syncData(
    ...
    try {
      if (typeof window.tiltExportAllData !== "function") {
        if (retryCount >= 3) {
          console.warn(
      ...
      const snapshot = await window.tiltExportAllData();
  ...
  async restoreData(
    ...
    try {
      ...
      if (response.ok && syncPayload.data) {
        if (typeof window.tiltImportDataFromObject === "function") {
          await window.tiltImportDataFromObject(
            syncPayload.data,
            (progressText: string) => {
              const loadingH1 =
          ...
          const exporter = window.tiltExportAllData;
  ...
  async performLogout(): Promise<void> {
    ...
    this.user = {};
    localStorage.removeItem("auth_user");
    localStorage.removeItem("tilt-sync-meta");
  ...
  async wipeLocalData(): Promise<void> {
    try {
      const preserveKeys = ["alertClosed", "tiltVisited", "tiltVersion"];
      ...
      localStorage.removeItem("tilt-bookmarks");
