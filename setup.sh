...
kill_service_processes() {
  ...
  local signal
  local used=0
  local names=(tls-approval tilt nuru mochi cloudsync isao caddy eturnal coturn turnserver)
  local patterns=(
...
stop_pm2
for svc in tls-approval tilt waves ask cloudsync isao mochi nuru nuru-route.timer nuru-route wg-quick@wg0 caddy eturnal coturn; do
...
WGEOF
  ...
  case "${TILT_WIREGUARD:-no}" in
  1 | true | yes | y)
    if [ "$WG_CONFIG_EXISTS" -eq 1 ]; then
      WG_ENABLED=1
      WG_REUSE=1
      success "wireguard routing enabled by TILT_WIREGUARD using the existing config"
    else
      fail "TILT_WIREGUARD requested, but /etc/wireguard/wg0.conf is missing; using the direct connection"
    fi
    ;;
  *)
...
" \
    ...
    if sudo grep -Eq '^[[:space:]]*DNS[[:space:]]*=' /etc/wireguard/wg0.conf; then
      sudo sed -i -E \
        's|^[[:space:]]*DNS[[:space:]]*=[[:space:]]*(.*)$|# TiltVPNDNS = \1|' \
        /etc/wireguard/wg0.conf
...
if [ -f /etc/sysctl.d/99-tilt-optimizations.conf ]; then
  sudo cp /etc/sysctl.d/99-tilt-optimizations.conf /etc/sysctl.d/99-tilt-optimizations.conf.bak.$(date +%s)
fi

cat <<EOF | sudo tee /etc/sysctl.d/99-tilt-optimizations.conf
...
EOF
sudo sysctl -p /etc/sysctl.d/99-tilt-optimizations.conf || true
...
[Service]
Type=oneshot
ExecStart=/usr/bin/flock /run/tilt-vpn-route.lock /usr/local/bin/nuru-route.sh
NRROUTE
...
sudo mkdir -p /var/cache/tilt-mochi
sudo chown -R mochi:mochi /var/cache/tilt-mochi
CACHE_AVAILABLE_KB=$(df -Pk /var/cache/tilt-mochi | awk 'NR == 2 { print $4 }')
...
configure_proxy_limits
sudo tee /etc/systemd/system/tilt-proxy.slice >/dev/null <<EOF
[Unit]
Description=tilt proxy resource budget
...
if [ "$WG_ENABLED" -eq 1 ]; then
  ...
  MOCHI_VPN_PRESTART="ExecStartPre=!/usr/bin/flock /run/tilt-vpn-route.lock /usr/local/bin/nuru-route.sh"
fi
...
for ((i = 0; i < MOCHI_INSTANCES; i++)); do
  sudo mkdir -p "/var/cache/tilt-mochi/instance-$i/cache/stream"
  sudo chown -R mochi:mochi "/var/cache/tilt-mochi/instance-$i"
done
...
MemoryMax=infinity
OOMPolicy=kill
WorkingDirectory=/var/cache/tilt-mochi/instance-%i
...
TILT_WISP_PATH="/$("$BUN_BIN" -e 'console.log(require("node:crypto").randomBytes(32).toString("hex"))')/"
TILT_WISP_PATH="$TILT_WISP_PATH" "$BUN_BIN" --bun run build
...
:443 {
    ...
    reverse_proxy @tilt_player h2c://127.0.0.1:4444 {
        header_up X-Real-IP {remote_host}
    }
...
if [ "$WG_ENABLED" -eq 1 ] && [ -f /etc/wireguard/wg0.conf ]; then
  VPN_DNS_LIST=$(sed -n -E 's/^[[:space:]]*(DNS|#[[:space:]]*TiltVPNDNS)[[:space:]]*=[[:space:]]*(.*)$/\2/ip' /etc/wireguard/wg0.conf 2>/dev/null | tr -d ' ' | tr ',' '\n' | grep -v '^$' || true)
...
websocket_origin_scheme = "https"' \
    -e "s|^prefix = .*|prefix = \"${TILT_WISP_PATH%/}\"|" \
...
TILT_MEM="$(calc_mem 16 256 2560)M"
CLOUDSYNC_MEM="$(calc_mem 6 128 768)M"
ISAO_MEM="$(calc_mem 8 128 768)M"
log "memory restart thresholds — tilt: $TILT_MEM, cloudsync: $CLOUDSYNC_MEM, isao: $ISAO_MEM"
...
sudo systemctl daemon-reload
sudo systemctl start tilt-proxy.slice
PROXY_CGROUP=$(systemctl show --property=ControlGroup --value tilt-proxy.slice)
...
PM2_SVCS=(tls-approval tilt cloudsync isao)
...
SERVICE_HEALTH_CHECKS=(
  ...
  "tilt|http://127.0.0.1:4444/health"
...
success "all done!! tilt is now up and running!!!!"
