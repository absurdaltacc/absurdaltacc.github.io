...
const SENSITIVE_TERM = /(games?|anime|proxy|rivet|folio|tilt)/i;
...
export function obfuscateSource(source, options, identifiersPrefix) {
  ...
  return encodeBuildLexemes(
    obfuscate(compatibleSource, {
      ...options,
      seed: identifiersPrefix || "tilt-build",
...
export default function tiltPlugin(
  buildId = createSourceBuildId(process.cwd()),
  wispPath = process.env.TILT_WISP_PATH ?? "/w/",
) {
  let outputDirectory;
  ...
  return {
    ...
    config(_config, { command }) {
      ...
      return {
        define: {
          __TILT_WISP_PATH__: JSON.stringify(command === "build" ? wispPath : "/w/"),
          __TILT_BUILD_ID__: JSON.stringify(command === "build" ? buildId : ""),
          __TILT_RUNTIME_PATHS__: JSON.stringify(command === "build"
